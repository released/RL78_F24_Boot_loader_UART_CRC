...
#define TIMEOUT_INTERVAL    	                        (30000)   // 30 sec timeout
volatile unsigned long boot_counter_tick = 0;
...
void boot_R_Config_TAU0_1_Stop(void);
void boot_R_Config_TAU0_1_Start(void);
...
#if ENALBE_TIMER_TAU0_1_IRQ     
	boot_R_Config_TAU0_1_Start();
#endif
...
#if ENALBE_TIMER_TAU0_1_IRQ
void boot_set_TimeoutFlag(unsigned char flag)
{
    timeout_flag = flag;
}

unsigned char boot_get_TimeoutFlag(void)
{
    return timeout_flag;
}

void boot_check_Timeout(void)
{
    if (boot_counter_tick > TIMEOUT_INTERVAL) 
    {                
        g_app_required_update = false;
        printf_tiny("(boot)Time-out, perform RST\r\n");
        // BSP_DI();
...
unsigned long boot_get_tick(void)
{
	return (boot_counter_tick);
}

void boot_Timer_1ms_IRQ(void)
{
    boot_counter_tick++;

    if ((boot_get_tick() % 1000) == 0)
    {
         if (boot_get_TimeoutFlag())
        {
            printf_tiny("(boot)entry counting :%ld\r\n" , boot_get_tick());
        }    
    }
...
void boot_R_Config_TAU0_1_Create(void)
{
...
void boot_ram_flag_test(void)
{
    ...
    // ram flag check
    printf_tiny("(boot)ram_flag(1):0x%0X",reset_to_bootloader>>16);
    printf_tiny("%X\r\n",reset_to_bootloader&0xFFFF);


    printf_tiny("(boot)_no_init_global:\r\n");
    for(i = 0 ; i < 16 ; i++)
    {
        printf_tiny("0x%02X,",_no_init_global[i]);
        if ((i+1)%8 ==0)
        {
            printf_tiny("\r\n");
        }          
...
void boot_main(void)
{
    ...
    g_app_required_update = false;
    _no_init_global[RAM_FLAG_INDICATE_BOOT_APP] = 0xAA;  
#if ENALBE_TIMER_TAU0_1_IRQ              
    boot_set_TimeoutFlag(1);   
	boot_R_Config_TAU0_1_Start();
#endif
...
#if ENALBE_TIMER_TAU0_1_IRQ
                    boot_set_TimeoutFlag(0);   // start to receive , no more counting 
#endif    
